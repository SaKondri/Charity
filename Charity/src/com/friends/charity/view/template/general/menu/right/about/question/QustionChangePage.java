...
public class QustionChangePage implements Serializable {
	...
	public String btnPage(ActionEvent actionEvent){
		getQustionList().setQustions(null);
		...
		if(size >= first){
			getQustionList().setQustions(getGeneralService().selectListValue(first -10, 10));
		}else if (size < first) {
			first = first-10;
			getQustionList().setQustions(getGeneralService().selectListValue(first, mod));
		}
		return null;
	}
}
