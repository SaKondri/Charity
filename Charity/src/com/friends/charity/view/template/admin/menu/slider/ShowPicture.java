...
public class ShowPicture implements Serializable{
	...
	public StreamedContent getPic() {
			String param =((String)((HttpServletRequest)  FacesContext.getCurrentInstance().getExternalContext().getRequest()).getParameter("param"));
			if(param != null){
				return new DefaultStreamedContent(new ByteArrayInputStream((getMap().get(Integer.parseInt(param)).getPic())));	
			}
			else {
